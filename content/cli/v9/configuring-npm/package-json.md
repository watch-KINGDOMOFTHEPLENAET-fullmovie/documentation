...
settings described in [`config`](/cli/v9/using-npm/config).
...
[`scope`](/cli/v9/using-npm/scope) for more detail.
...
* `package-lock.json` (use
  [`npm-shrinkwrap.json`](/cli/v9/configuring-npm/npm-shrinkwrap-json) if you wish
  it to be published)
...
See [folders](/cli/v9/configuring-npm/folders#executables) for more info on
executables.
...
See [`scripts`](/cli/v9/using-npm/scripts) to find out more about writing package
scripts.
...
* `tag` A specific version tagged and published as `tag`  See [`npm
  dist-tag`](/cli/v9/commands/npm-dist-tag)
...
param.  See [`config`](/cli/v9/using-npm/config) for more on the topic.
...
[`engine-strict` config](/cli/v9/using-npm/config#engine-strict) flag, this field is
...
See [`config`](/cli/v9/using-npm/config) to see the list of config options that
can be overridden.
...
up to find each [workspace](/cli/v9/using-npm/workspaces) that needs to be
...
See [`workspaces`](/cli/v9/using-npm/workspaces) for more examples.
